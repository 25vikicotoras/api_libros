...
router.post('/', (req, res, next) => {
    try {
        ...
        if (error) {
            const validationError = new Error('Error de valadación');
...
router.put('/: id', (req, res, next) => {
    try {
        ...
        libros.titulo = titulo || libro.titulo;
...
router.delete('/:id', (req, res, next) => {
    try {
        ...
        if (index === -1) {
            const error = new Error('Libro no encintrado');
            error.status = 404;
            throw error;
        }
